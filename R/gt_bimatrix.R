...
#' @param X Payoff matrix. Defaults to matching pennies.
#' @keywords 
#' @export
#' @examples
#' gt_bimatrix()
...
gt_bimatrix <- function(
            X = matrix(c(0,1,1,0),2),              # Payoff matrix
            Y=t(X), 
						P1="Player 1", 
						P2="Player 2", 
						labels1=NULL, 
						...
						maincex=1, 
						col.main="red", 
						mainline= NA  # Arguments for plot title
						){ 
	...
	if (is.null(labels2)) labels2 <- mapply(paste, "Y",1:ncol(X), sep="")
	if (length(labels1)!=nrow(X)|length(labels2)!=ncol(X)) stop ("Labels do not match options")
